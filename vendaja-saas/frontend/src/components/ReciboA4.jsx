...
const ReciboA4 = ({ venda, configLoja, fechar }) => {
  ...
  const DocumentoPagina = ({ tipo }) => (
    <div className="bg-white p-16 mb-8 mx-auto w-[210mm] min-h-[297mm] shadow-none print:shadow-none print:m-0 print:break-after-page flex flex-col">
      
      {/* CABEÇALHO DE ALTO NÍVEL */}
      <div className="flex justify-between items-start border-b-4 border-slate-900 pb-10 mb-10">
        <div className="flex gap-6 items-center">
          {configLoja.logoUrl ? (
            <img src={configLoja.logoUrl} alt="Logo" className="h-24 w-24 object-contain rounded-xl" />
          ) : (
            <div className="h-24 w-24 bg-slate-900 rounded-2xl flex items-center justify-center text-white font-black text-3xl">
              {configLoja.nome?.substring(0, 1).toUpperCase()}
            </div>
          )}
          <div className="space-y-1">
            <h1 className="text-3xl font-black uppercase tracking-tighter text-slate-900 leading-none">
              {configLoja.nome || "NOME DO NEGÓCIO"}
            </h1>
            <div className="text-[11px] font-bold text-slate-500 uppercase tracking-wider space-y-0.5">
              ...
              <p className="flex items-center gap-2"><ShieldCheck size={12}/> NUIT: {configLoja.nuit || "000000000"}</p>
              <p className="flex items-center gap-2"><Phone size={12}/> {configLoja.telefone || "Contacto"}</p>
            </div>
          </div>
        </div>
        ...
        <div className="text-right">
          ...
          <div className="text-[12px] font-black text-slate-900 uppercase">
            ...
            <p className="mt-2 text-slate-500 font-bold">Data: {new Date(venda.data).toLocaleDateString('pt-MZ')}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-50 rounded-[2rem] p-8 mb-10 flex justify-between items-center border border-slate-100">
        <div>
          ...
          <p className="text-lg font-black text-slate-900 uppercase">{venda.infoAdicional || "Consumidor Final"}</p>
          <p className="text-xs font-bold text-slate-500 mt-1 uppercase">NUIT: {venda.clienteNuit || "999-999-999"}</p>
        </div>
          ...
          <tbody className="divide-y divide-slate-100">
            {venda.itens.map((item, idx) => (
              <tr key={idx} className="text-[11px] font-bold text-slate-700">
                ...
                <td className="py-6 px-6 text-center text-slate-400 italic">{item.qtd}</td>
                <td className="py-6 px-6 text-right">{item.preco.toFixed(2)}</td>
                <td className="py-6 px-6 text-right font-black text-slate-900">{ (item.preco * item.qtd).toFixed(2) }</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="flex justify-between items-end">
          <div className="space-y-4">
            <div className="bg-slate-100 px-6 py-4 rounded-2xl inline-block border border-slate-200">
               ...
               <p className="text-sm font-black text-slate-900 uppercase italic tracking-widest">{venda.metodo}</p>
            </div>
            <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest leading-loose">
              <p>• Mercadoria entregue após pagamento</p>
              <p>• Documento processado por software certificado</p>
            </div>
          </div>

          <div className="w-full max-w-[320px] space-y-2">
            <div className="flex justify-between text-xs font-bold text-slate-500 uppercase px-2">
              <span>Valor Líquido (Subtotal)</span>
              <span>{(venda.total / 1.16).toFixed(2)}</span>
            </div>
            <div className="flex justify-between text-xs font-bold text-slate-500 uppercase px-2">
              <span>IVA Aplicado (16%)</span>
      ...
      <div className="grid grid-cols-2 gap-20 mt-16 text-center">
        <div>
          ...
          <p className="text-[9px] font-black uppercase text-slate-400 tracking-[0.3em]">Assinatura Autorizada</p>
        </div>
        <div>
          <div className="h-px bg-slate-200 w-full mb-4"></div>
      ...
      {/* RODAPÉ DO SISTEMA (DISCRETO) */}
      <div className="mt-auto pt-10 flex justify-center items-center gap-2 opacity-30 grayscale hover:grayscale-0 transition-all cursor-default print:opacity-20">
        <FileText size={10} />
        <p className="text-[8px] font-black uppercase tracking-[0.5em]">Processado por SISTEMA PRO - Gestão Inteligente de Negócios</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-slate-900/90 backdrop-blur-md overflow-y-auto pt-10 pb-20">
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        /* Força a tipografia profissional */
