...
const LojaPublica = () => {
  ...
  const totalProd = carrinho.reduce((acc, i) => acc + (i.precoVenda * i.qtd), 0);
  ...
  const finalizarPedido = () => {
    ...
    // Formatar dados do pagamento para o WhatsApp
    ...
    window.open(`https://wa.me/${loja.telefone?.replace(/\s+/g, '')}?text=${texto}`, '_blank');
  };
    ...
    <div className="min-h-screen bg-slate-50 pb-32">
      ...
      <div className="bg-white p-8 rounded-b-[3rem] shadow-sm text-center border-b border-slate-100">
        <div className="max-w-4xl mx-auto flex flex-col items-center">
          ...
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2 flex items-center gap-2"><MapPin size={12}/> {loja.endereco}</p>
        </div>
      </div>
        ...
        {produtos.map(p => (
          <div key={p.id} className="bg-white p-4 rounded-[2.5rem] flex items-center gap-4 border border-slate-100 shadow-sm">
            ...
            <div className="flex-1">
              ...
              <p className="text-blue-600 font-black text-lg mt-1">{p.precoVenda} <span className="text-[10px] uppercase font-bold text-slate-400">{loja.moeda || 'MT'}</span></p>
            </div>
            <button onClick={() => adicionarAoCarrinho(p)} className="bg-blue-600 text-white p-4 rounded-2xl hover:scale-105 active:scale-95 transition-all"><ShoppingBag size={20}/></button>
          </div>
        ))}
      </div>
      ...
      {carrinho.length > 0 && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[90%] max-w-lg bg-slate-900 text-white rounded-[2.5rem] p-4 shadow-2xl flex items-center justify-between animate-in slide-in-from-bottom-5">
          <div className="pl-4">
            <p className="text-[10px] font-black uppercase text-slate-500 tracking-widest">Total</p>
            <p className="text-xl font-black italic">{totalProd} {loja.moeda}</p>
          </div>
          <button onClick={() => setModalAberto(true)} className="bg-blue-600 px-8 py-4 rounded-[1.8rem] font-black uppercase text-xs tracking-[0.2em] flex items-center gap-2">Finalizar <ChevronRight size={18}/></button>
        </div>
      )}
